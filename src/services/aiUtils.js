...
const safeScalarText = (value, fallback = '') => typeof value === 'number' && Number.isFinite(value) ? String(value) : safeText(value, fallback);
const hasVehicleIdentity = (vehicle) => ['year', 'make', 'model'].every((key) => Boolean(safeScalarText(vehicle?.[key])));
const hasValidVin = (value) => /^[A-HJ-NPR-Z0-9]{17}$/.test(safeScalarText(value).replace(/[^A-Z0-9]/gi, '').toUpperCase());
const getAskingPrice = (value) => safeMoney(Number(safeScalarText(value).replace(/[^0-9.]/g, '')));
...
export const getEvidenceAudit = (input = {}) => {
  ...
  if (hasVehicleIdentity(vehicle)) confirmed.unshift('Vehicle identity');
  ...
  if (!getAskingPrice(vehicle.asking)) missing.push('asking price');
...
export const getAiQualitySummary = (input = {}) => {
  ...
  const identityBonus = hasValidVin(vehicle.vin) ? 10 : 0;
  ...
  if (hasValidVin(vehicle.vin)) drivers.push('VIN identified');
...
export const buildAiAnalysis = (input = {}) => {
  ...
  const confidence = clamp(Math.round(35 + evidence.score * 0.55 + (hasValidVin(vehicle.vin) ? 10 : 0)), 35, 95);
  ...
  const asking = getAskingPrice(vehicle.asking);
