...
export const getReportReadiness = (input = {}) => { const safeInput = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const { vehicle = {}, checklist = {}, photos = [] } = safeInput; const safeVehicle = isRecord(vehicle) ? vehicle : {}; const safePhotos = (Array.isArray(photos) ? photos : []).filter(isRecord); const missing = []; if (!/^\d{4}$/.test(String(safeVehicle.year || '').trim()) || !String(safeVehicle.make || '').trim() || !String(safeVehicle.model || '').trim()) missing.push('vehicle details'); if (countCompletedChecklistSections(checklist) < 5) missing.push('checklist'); if (!safePhotos.length) missing.push('photo evidence'); return { ready: missing.length === 0, missing }; };
