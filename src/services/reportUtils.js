...
export const getMainFlowReadiness = (input = {}) => { const safeInput = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const { vehicle = {}, checklist = {}, photos = [] } = safeInput; const safePhotos = Array.isArray(photos) ? photos : []; const missing = []; if (!hasVehicleDetails(vehicle)) missing.push('vehicle details'); if (countCompletedChecklistSections(checklist) < 5) missing.push('checklist'); if (!safePhotos.length) missing.push('photo evidence'); return { ready: missing.length === 0, missing }; };
