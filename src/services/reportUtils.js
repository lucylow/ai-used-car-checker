...
export const getMainFlowReadiness = ({ vehicle = {}, checklist = {}, photos = [] } = {}) => { const safeChecklist = checklist && typeof checklist === 'object' && !Array.isArray(checklist) ? checklist : {}; const safePhotos = Array.isArray(photos) ? photos : []; const missing = []; if (!hasVehicleDetails(vehicle)) missing.push('vehicle details'); if (Object.keys(safeChecklist).filter((key) => safeChecklist[key]).length < 5) missing.push('checklist'); if (!safePhotos.length) missing.push('photo evidence'); return { ready: missing.length === 0, missing }; };
