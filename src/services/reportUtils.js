...
export const getLocalSaveSuccessLabel = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const { pendingCleanupFailed = false, payloadLength = 0 } = safe; return pendingCleanupFailed ? 'Saved locally, but an older pending copy could not be cleared. Retry later.' : payloadLength > 250000 ? 'Inspection is large — keeping a compact local copy' : 'Saved locally'; };
...
export const getLocalRecoveryBanner = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const count = getSafeCount(safe.retryCount); const saveRetry = Boolean(safe.saveRetry); if (!count && !saveRetry) return null; return { title: 'Local recovery needed', body: `${count || 1} local operation${(count || 1) === 1 ? '' : 's'} is waiting for retry. Your inspection remains available on this device.`, action: 'Retry local saves' }; };
