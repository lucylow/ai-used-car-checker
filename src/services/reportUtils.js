...
export const getLocalSaveIndicator = ({ state = 'idle', retry = false } = {}) => ({ label: retry ? 'Local recovery needed' : getLocalSaveLabel(state), tone: retry || state === 'error' ? 'warning' : state === 'saving' ? 'active' : state === 'saved' ? 'success' : 'neutral' });
...
export const getLocalSaveSuccessLabel = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const pendingCleanupFailed = safe.pendingCleanupFailed === true; const payloadLength = safe.payloadLength; return pendingCleanupFailed ? 'Saved locally, but an older pending copy could not be cleared. Retry later.' : payloadLength > 250000 ? 'Inspection is large — keeping a compact local copy' : 'Saved locally'; };
...
export const getLocalRecoveryBanner = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const count = getSafeCount(safe.retryCount); const saveRetry = safe.saveRetry === true; if (!count && !saveRetry) return null; return { title: 'Local recovery needed', body: `${count || 1} local operation${(count || 1) === 1 ? '' : 's'} is waiting for retry. Your inspection remains available on this device.`, action: 'Retry local saves' }; };
...
const getSafeAttemptCount = (value) => { const numeric = Number(value); return Number.isFinite(numeric) ? Math.min(100, Math.max(0, Math.floor(numeric))) : 0; };
export const buildDiagnosticExport = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; return JSON.stringify({ app: 'carwise', diagnosticVersion: 1, appVersion: safeReportText(safe.appVersion, 'unknown', 40), exportedAt: new Date().toISOString(), privacy: 'Contains operational diagnostics only; no VIN, vehicle, issue, or photo data.', recoveryLog: normalizeRecoveryLog(safe.recoveryLog), retryQueue: (Array.isArray(safe.retryDiagnostics) ? safe.retryDiagnostics : []).filter(isRecord).slice(-6).map((item) => ({ key: safeReportText(item.key, 'unknown', 80), attempts: getSafeAttemptCount(item.attempts), maxAttempts: getSafeAttemptCount(item.maxAttempts), detail: safeReportText(item.detail, 'Needs attention or retry.', 160) })) }, null, 2); };
export const getSafeDateLabel = (value, fallback = 'Date unavailable', options = {}) => { if (typeof value !== 'string' || !value.trim() || Number.isNaN(Date.parse(value))) return fallback; try { return new Date(value).toLocaleDateString(undefined, options); } catch { return fallback; } };
...
export const getDurablePhotoFileName = (fileName = 'inspection.jpg', timestamp = Date.now()) => {
  const safeName = safeReportText(fileName, 'inspection.jpg', 160).replace(/[^a-zA-Z0-9._-]/g, '_') || 'inspection.jpg';
  const numericTimestamp = Number(timestamp);
  const safeTimestamp = Number.isFinite(numericTimestamp) ? Math.max(0, Math.floor(numericTimestamp)) : Date.now();
  return `carwise-${safeTimestamp}-${safeName}`;
};
...
export const getMotionDuration = (baseDuration, intensity = 'standard', reduceMotion = false) => { if (reduceMotion) return 0; const numericBase = Number(baseDuration); const base = Number.isFinite(numericBase) ? Math.max(0, numericBase) : 0; const multiplier = intensity === 'gentle' ? 1.35 : intensity === 'lively' ? 0.7 : 1; const duration = Math.round(base * multiplier); return Math.min(2000, Math.max(40, Number.isFinite(duration) ? duration : 40)); };
...
export const normalizeCarwiseSettings = (value, fallback = { compactMode: false, aiDisclosure: true, motionIntensity: 'standard', offlineDemoData: true, failureToggles: {} }) => { const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}; const safeFallback = fallback && typeof fallback === 'object' && !Array.isArray(fallback) ? fallback : {}; const sourceToggles = source.failureToggles && typeof source.failureToggles === 'object' && !Array.isArray(source.failureToggles) ? source.failureToggles : {}; const fallbackToggles = safeFallback.failureToggles && typeof safeFallback.failureToggles === 'object' && !Array.isArray(safeFallback.failureToggles) ? safeFallback.failureToggles : {}; const motionIntensity = ['gentle', 'standard', 'lively'].includes(source.motionIntensity) ? source.motionIntensity : ['gentle', 'standard', 'lively'].includes(safeFallback.motionIntensity) ? safeFallback.motionIntensity : 'standard'; return { compactMode: Boolean(source.compactMode), aiDisclosure: source.aiDisclosure !== false, motionIntensity, offlineDemoData: source.offlineDemoData !== false, failureToggles: { camera: Boolean(sourceToggles.camera ?? fallbackToggles.camera), photo: Boolean(sourceToggles.photo ?? fallbackToggles.photo), report: Boolean(sourceToggles.report ?? fallbackToggles.report), storage: Boolean(sourceToggles.storage ?? fallbackToggles.storage) } }; };
export const isSettingsPersistenceReady = (input = {}) => Boolean(input && typeof input === 'object' && !Array.isArray(input) && input.settingsReady === true);
export const shouldScheduleLocalPersistence = (input = {}) => Boolean(input && typeof input === 'object' && !Array.isArray(input) && input.restored === true);
export const isCurrentPersistenceGeneration = (input = {}) => { if (!input || typeof input !== 'object' || Array.isArray(input)) return false; const generation = Number(input.generation); const currentGeneration = Number(input.currentGeneration); return Number.isInteger(generation) && generation >= 0 && Number.isInteger(currentGeneration) && currentGeneration >= 0 && generation === currentGeneration; };
export const isCurrentActionGeneration = (input = {}) => { if (!input || typeof input !== 'object' || Array.isArray(input)) return false; const generation = Number(input.generation); const currentGeneration = Number(input.currentGeneration); return Number.isInteger(generation) && generation >= 0 && Number.isInteger(currentGeneration) && currentGeneration >= 0 && generation === currentGeneration; };
export const getTransientTimerCleanupKeys = () => ['ai', 'reportAction', 'persist', 'undo', 'findingUndo', 'noteUndo'];
export const getNextPersistenceGeneration = (generation) => { const numeric = Number(generation); return Number.isSafeInteger(numeric) && numeric >= 0 && numeric < Number.MAX_SAFE_INTEGER ? numeric + 1 : 0; };
...
export const getInspectionActionGuidance = (input = {}) => { const safe = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const photoCount = getSafeCount(safe.photoCount); const reportReady = Boolean(safe.reportReady); if (!reportReady) return photoCount ? 'Finish the required sections before sharing your report.' : 'Complete the checklist and add photo evidence before sharing.'; return photoCount ? `${photoCount} photo${photoCount === 1 ? '' : 's'} attached · report ready to share.` : 'Report ready to share; add photos for stronger evidence.'; };
...
const countCompletedChecklistSections = (checklist = {}) => { const safeChecklist = checklist && typeof checklist === 'object' && !Array.isArray(checklist) ? checklist : {}; return checklistSectionAliases.filter((aliases) => aliases.some((alias) => Object.keys(safeChecklist).some((key) => String(key).trim().toLowerCase() === alias && Boolean(safeChecklist[key])))).length; };
export const getMainFlowReadiness = (input = {}) => { const safeInput = input && typeof input === 'object' && !Array.isArray(input) ? input : {}; const { vehicle = {}, checklist = {}, photos = [] } = safeInput; const safePhotos = (Array.isArray(photos) ? photos : []).filter((photo) => photo && typeof photo === 'object' && !Array.isArray(photo)); const missing = []; if (!hasVehicleDetails(vehicle)) missing.push('vehicle details'); if (countCompletedChecklistSections(checklist) < 5) missing.push('checklist'); if (!safePhotos.length) missing.push('photo evidence'); return { ready: missing.length === 0, missing }; };
