...
export const getMainFlowReadiness = ({ vehicle = {}, checklist = {}, photos = [] } = {}) => { const missing = []; if (!vehicle.make?.trim() || !vehicle.model?.trim()) missing.push('vehicle details'); if (Object.keys(checklist).filter((key) => checklist[key]).length < 5) missing.push('checklist'); if (!photos.length) missing.push('photo evidence'); return { ready: missing.length === 0, missing }; };
