...
export default function RedesignShell({ screen = 'home', state = {}, actions = {}, showNavigation = true }) {
  ...
  const nav = {
    ...actions,
    onOpenCopilot: () => setAssistantVisible(true),
    onNotifications: actions.onNotifications || (() => actions.onOpenSettings?.()),
    onOpenPhoto: (photo) => {
      setSelectedPhoto(photo);
      actions.onOpenPhoto?.(photo);
    },
  };
  const setTab = (tab) => {
    setActiveTab(tab);
    ...
    if (tab === 'history') actions.onNavigate?.('historyTab');
  ...
  return <View style={{ flex: 1 }}>
    ...
    <FullscreenEvidenceViewer
      ...
      photos={data.photos}
      initialIndex={Math.max(0, data.photos.findIndex((photo) => photo.id === selectedPhoto?.id))}
      onClose={() => setSelectedPhoto(null)}
      onSelectFinding={(finding) => actions.onOpenFinding?.(finding)}
    />
  </View>;
}
