...
export default function UXHeuristicsV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="UX heuristics" eyebrow="QUALITY" subtitle="A self-review surface for hierarchy, feedback, consistency and error recovery." onBack={onBack}/>

{[['Hierarchy','A clear primary action appears first.'],['Feedback','Every async action exposes state.'],['Consistency','Risk, confidence and evidence use shared components.'],['Recovery','Failed requests never erase saved inspection state.'],['Trust','AI estimates are explicitly labeled.'],['Accessibility','Essential information is not encoded by color alone.']].map(([x,d])=><Surface key={x} style={{marginBottom:8}}><Text variant="h3">{x}</Text><Text muted style={{marginTop:4}}>{d}</Text></Surface>)}
