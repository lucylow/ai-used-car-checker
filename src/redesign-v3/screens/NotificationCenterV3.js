...
export default function NotificationCenterV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Notification center" eyebrow="UPDATES" subtitle="Surface only high-signal notifications with clear next actions." onBack={onBack}/>

<NotificationCard title="AI analysis ready" message="Four findings are waiting for review." tone="info" onPress={()=>onNavigate?.('ai')}/>
<NotificationCard title="Evidence requested" message="A clearer dashboard image may improve confidence." tone="watch" onPress={()=>onNavigate?.('camera')}/>
<NotificationCard title="Contract ready" message="Review terms before opening human signature." tone="info" onPress={()=>onNavigate?.('contract')}/>
<NotificationCard title="Market update" message="Comparable listings moved in your area." tone="success" onPress={()=>onNavigate?.('market')}/>
