...
export default function VINManualV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Enter VIN" eyebrow="VIN" subtitle="A clean fallback when scanning is unavailable." onBack={onBack}/>

<Surface><Badge label="17 CHARACTERS" tone="info"/><Text variant="h2" style={{marginTop:10}}>Enter the VIN</Text><Text muted style={{marginTop:4}}>Use the VIN printed on the vehicle or title paperwork.</Text><View style={{marginTop:14}}><Field label="VIN" placeholder="1HGCV2F34LA000000" mono/></View><Text variant="caption" muted style={{marginTop:2}}>Avoid spaces or hyphens.</Text></Surface>
<SectionHeader title="Privacy hint"/><Surface><Text muted>Keep document and identity details visible only where necessary for the inspection workflow.</Text></Surface>
<View style={{marginTop:15}}><Button title="Verify VIN" onPress={()=>onNavigate?.('market')}/></View>
