...
export default function AppStoreOverviewV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="App Store overview" eyebrow="RELEASE" subtitle="A visual checklist for the public-facing product surface." onBack={onBack}/>

<AppStoreBadge label="Designed for iPhone"/>
<SectionHeader title="Primary screenshots"/>{['Instant Price Intelligence','AI-Powered Condition Scan','Complete Inspection in Minutes','From Report to Signed Contract','Track Every Inspection','Go Pro — Advanced Tools'].map((x,i)=><Surface key={x} style={{marginBottom:8}}><Badge label={`SCREEN ${i+1}`} tone="info"/><Text variant="h2" style={{marginTop:8}}>{x}</Text><Text muted style={{marginTop:4}}>Keep one visual story and one action per screenshot.</Text></Surface>)}
<SectionHeader title="Public-facing trust"/><AITrustBanner/>
