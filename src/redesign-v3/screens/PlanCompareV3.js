...
export default function PlanCompareV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Plan comparison" eyebrow="SUBSCRIPTION" subtitle="Keep monetization transparent and easy to understand." onBack={onBack}/>

<SectionHeader title="Free"/><Surface><Badge label="FREE" tone="neutral"/><View style={{marginTop:9}}>{['3 inspections','Basic report','Photo evidence'].map(x=><ActionRow key={x} icon="checkmark" title={x} tone={COLORS.mint}/>)}</View></Surface>
<SectionHeader title="Pro"/><SubscriptionOffer onPress={()=>{}}/>
<SectionHeader title="Restore"/><RestorePurchasesRow onPress={()=>{}}/>
<SupportCard onPress={()=>onNavigate?.('help')}/>
