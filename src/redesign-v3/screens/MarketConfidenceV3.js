...
export default function MarketConfidenceV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Market confidence" eyebrow="MARKET" subtitle="Show the confidence behind the pricing context." onBack={onBack}/>

<Surface><Text variant="caption" muted>Market confidence</Text><Text variant="display">92%</Text><ConfidencePill value={.92}/><Text muted style={{marginTop:9}}>Confidence describes the consistency of the current comparable set, not a guarantee of a vehicle value.</Text></Surface>
<SectionHeader title="Inputs"/>{['Vehicle identity','Local comparables','Mileage similarity','Listing freshness'].map(x=><ActionRow key={x} icon="checkmark-circle" title={x} subtitle="Included" tone={COLORS.mint}/>) }
<SectionHeader title="What could change it?"/><View style={{gap:8}}>{['A broader search area','Different seller type','New listings','Condition evidence'].map(x=><ActionRow key={x} icon="options-outline" title={x} subtitle="Can shift the estimate" tone={COLORS.amber}/>)}</View>
