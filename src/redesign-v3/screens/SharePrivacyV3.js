...
export default function SharePrivacyV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Sharing privacy" eyebrow="SHARING" subtitle="Make shared content deliberate and understandable." onBack={onBack}/>

<Surface><Badge label="SHARED VIEW" tone="info"/><Text variant="h2" style={{marginTop:10}}>Choose what the recipient sees</Text><Text muted style={{marginTop:4}}>Public reports should omit unnecessary personal information and focus on vehicle evidence.</Text></Surface>
<SectionHeader title="Recommended to share"/>{['Vehicle identity','Key findings','Market context','Repair estimate','Certificate status'].map(x=><ChoiceCard key={x} title={x} subtitle="Included" selected onPress={()=>{}}/>) }
<SectionHeader title="Keep private"/>{['Personal contact details','Private notes','Unrelated documents'].map(x=><ChoiceCard key={x} title={x} subtitle="Excluded by default" selected={false} onPress={()=>{}}/>) }
<View style={{marginTop:15}}><Button title="Preview share" onPress={()=>onNavigate?.('share')}/></View>
