...
export default function ContractDisclosureV3({onBack,onNavigate}){
 const [active,setActive]=useState('Overview');
 return <Screen><ScreenHeader title="Contract disclosures" eyebrow="CONTRACT" subtitle="Make known issues easy to find before signing." onBack={onBack}/>

<ContractSummary/>
<SectionHeader title="Disclosed findings"/>{FINDINGS.map((f,i)=><Surface key={f.id} style={{marginBottom:8}}><View style={{flexDirection:'row',justifyContent:'space-between'}}><Text variant="bodyStrong">{f.title}</Text><Badge label={i===3?'Critical':'Disclosed'} tone={i===3?'danger':'watch'}/></View><Text variant="caption" muted style={{marginTop:5}}>{f.area} · linked inspection evidence</Text></Surface>)}
<SectionHeader title="Human review"/><SafetyWarning children="Review all disclosures and seek professional or legal advice where appropriate before signing."/>
<View style={{marginTop:15}}><Button title="Continue to signature" onPress={()=>onNavigate?.('signature')}/></View>
