...
test('guards delayed AI and photo success callbacks against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /aiTimer\.current = setTimeout\(\(\) => \{ if \(!mountedRef\.current \|\| !isCurrentActionGeneration/);
  assert.match(appSource, /if \(!mountedRef\.current \|\| !isCurrentActionGeneration\(\{ generation: actionGeneration, currentGeneration: persistGeneration\.current \}\)\) return; setPhotos/);
});
...
test('guards onboarding step transitions before state updates after unmount', () => {
  ...
  assert.match(appSource, /const goToNextOnboardingStep = \(\) => \{ if \(!mountedRef\.current\) return; setOnboardingStep/);
  assert.match(appSource, /onPress=\{goToPreviousOnboardingStep\}/);
  assert.match(appSource, /onPress=\{goToNextOnboardingStep\}/);
});
...
test('guards direct AI finding mutation entries against unmounted state updates', () => {
  ...
  assert.match(appSource, /const dismissAiFindings = \(\) => \{ if \(!mountedRef\.current \|\| !canReviewAiFindings/);
  assert.match(appSource, /const addCustomFinding = \(\) => \{ if \(!mountedRef\.current\) return;/);
});

test('guards issue-edit save entry against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /const saveIssueEdit = \(\) => \{ if \(!mountedRef\.current \|\| !editingIssue\) return;/);
});

test('guards tool-note save entry against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /const saveToolNote = \(type, note, source\) => \{ if \(!mountedRef\.current\) return; setToolNotes\(/);
});
...
test('guards backup and photo viewer close callbacks against unmounted state updates', () => {
  ...
  assert.match(appSource, /const closeBackupPreview = \(\) => \{ if \(!mountedRef\.current\) return;/);
});
...
test('guards direct finding and tool-note mutations against unmounted state updates', () => {
  ...
  assert.match(appSource, /const deleteToolNote = \(type\) => \{ if \(!mountedRef\.current\) return;/);
});

test('guards report preview close callbacks against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /const closeReportPreview = \(\) => \{ if \(!mountedRef\.current\) return; const next = getReportPreviewCloseState\(\);/);
});
...
test('guards history and undo callbacks against unmounted state updates', () => {
  ...
  assert.match(appSource, /const executeHistoryAction = \(\) => \{ if \(!mountedRef\.current \|\| !historyConfirm\) return;/);
  assert.match(appSource, /const undoRemovedFinding = \(\) => \{ if \(!mountedRef\.current \|\| !findingUndoItem\) return;/);
  assert.match(appSource, /const undoDeletedToolNote = \(\) => \{ if \(!mountedRef\.current \|\| !noteUndoItem\) return;/);
});

test('guards shared recovery logging against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /const recordRecoveryEvent = \(operation, outcome, detail\) => \{ if \(!mountedRef\.current\) return; setRecoveryLog\(/);
});
...
test('guards VIN camera callbacks after permission and capture awaits', () => {
  ...
  assert.match(vinSource, /launchCameraAsync\(\{ mediaTypes: \['images'\][\s\S]*?\}\); if \(!mountedRef\.current\) return; if \(captured\.canceled/);
});
...
test('guards internal report share and PDF export awaits against unmounted state updates', () => {
  ...
  assert.match(appSource, /await Share\.share\(\{ message: report, title: 'Carwise inspection report' \}\); if \(!mountedRef\.current\) return;/);
  assert.match(appSource, /const reportPhotos = await Promise\.all\([\s\S]{0,280}if \(!mountedRef\.current\) return;/);
  assert.match(appSource, /await Sharing\.shareAsync\(result\.uri, \{ mimeType: 'application\/pdf'/);
  assert.match(appSource, /catch \(error\) \{ if \(!mountedRef\.current\) return; setSaveStatus\(`\$\{getRecoveryGuidance\('report'\)\}/);
});
...
test('guards diagnostic sharing and backup export catch paths against unmounted state updates', () => {
  ...
  assert.match(appSource, /catch \(error\) \{ if \(!mountedRef\.current\) return; const detail = getErrorDetail\(error, 'diagnostic export failed'\)/);
  assert.match(appSource, /catch \(error\) \{ if \(!mountedRef\.current\) return; const detail = getErrorDetail\(error, 'backup export failed'\)/);
});

test('guards asynchronous backup flows against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /if \(!mountedRef\.current\) return; if \(await Sharing\.isAvailableAsync\(\)\)/);
  assert.match(appSource, /FileSystem\.readAsStringAsync\(result\.assets\[0\]\.uri/);
  assert.match(appSource, /if \(!mountedRef\.current\) return;/);
  assert.match(appSource, /catch \(error\) \{ if \(!mountedRef\.current\) return; const detail = getErrorDetail\(error, 'backup restore failed'\)/);
});
...
test('guards checklist navigation callbacks against unmounted state updates', () => {
  const appSource = readFileSync(new URL('../App.js', import.meta.url), 'utf8');
  assert.match(appSource, /const goChecklist = \(\) => \{ if \(!mountedRef\.current\) return; setScreen\('checklist'\); \}/);
