...
import { getPhotoCount } from '../src/services/uiUtils';
...
function InspectionChecklist({ checklist, checklistComplete, photos, onToggleSection, onBack, onPhotos, onSave, MotionButton, styles, colors }) {
  const photoCount = getPhotoCount(photos);
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <Text style={styles.muted}>{getInspectionActionGuidance({ photoCount, reportReady: checklistComplete === 5 })}</Text>
    <TouchableOpacity accessibilityRole="button" accessibilityLabel="Manage inspection photos" style={styles.secondaryButton} onPress={onPhotos}><Text style={styles.secondaryButtonText}>{photoCount ? `Review ${photoCount} photo${photoCount === 1 ? '' : 's'}` : 'Add photo evidence'}</Text></TouchableOpacity>
