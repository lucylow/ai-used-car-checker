...
import { normalizeMarketComparison, getMarketComparisonSummary } from '../src/services/marketUtils';
...
function SavedInspectionDetail({ item, restoreSource = '', onBack, onSelectPhoto, onOpenEditableReport, Card, Pill, styles }) {
  ...
  const market = normalizeMarketComparison(item.marketComparison);
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <Card><Text style={styles.cardEyebrow}>MARKET COMPARISON</Text><Text style={styles.muted}>Asking ${market.askingPrice.toLocaleString()} · Comparable ${market.comparableLow.toLocaleString()}–${market.comparableHigh.toLocaleString()}</Text><Text style={styles.muted}>Condition: {market.condition} · Mileage: {market.mileage ? market.mileage.toLocaleString() : 'Not provided'}</Text><Text style={styles.muted}>{getMarketComparisonSummary(market)}</Text></Card>
