...
function VinTool({ onHome, onUse, Card, ActionButton, colors, styles }) {
  ...
  const loadDemoVehicle = () => { if (busy || cameraBusy) return; setResult(getVinMockFallback(value)); };
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    {result?.status === 'fallback' ? <Card style={styles.resultCard}><Text style={styles.cardEyebrow}>OFFLINE FALLBACK</Text><Text style={styles.muted}>No vehicle details were changed. You can retry the lookup or continue entering details manually.</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Load offline demo vehicle" style={styles.secondaryButton} onPress={loadDemoVehicle}><Text style={styles.secondaryButtonText}>Load offline demo vehicle</Text></TouchableOpacity></Card> : null}
