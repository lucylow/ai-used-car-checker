...
import { canApplyDecodedVehicle, decodeVin, getVinResultCompleteness, getSafeDecodedVehicle, getVinMockFallback, canConfirmVinCapture, getVinCaptureConfidence, getVinConfidenceDisclosure, normalizeVinCandidate } from '../src/services/vinService';
import { getInspectionNavigationLabel, getToolInputGuidance, getVinWalkthroughStep } from '../src/services/reportUtils';

function VinTool({ onHome, onUse, Card, ActionButton, colors, styles, allowDemoData = true }) {
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    {result?.status === 'fallback' ? <Card style={styles.resultCard}><Text style={styles.cardEyebrow}>OFFLINE FALLBACK</Text><Text style={styles.muted}>No vehicle details were changed. You can retry the lookup or continue entering details manually.</Text><View style={{ flexDirection: 'row', gap: 10, marginTop: 10 }}><View style={{ flex: 1 }}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Retry live VIN lookup" style={styles.secondaryButton} onPress={runDecode}><Text style={styles.secondaryButtonText}>Retry live lookup</Text></TouchableOpacity></View>{allowDemoData ? <View style={{ flex: 1 }}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Load offline demo vehicle" style={styles.secondaryButton} onPress={loadDemoVehicle}><Text style={styles.secondaryButtonText}>Load offline demo</Text></TouchableOpacity></View> : null}</View></Card> : null}
    {result?.status === 'mock' ? <Card style={styles.resultCard}><Text style={styles.cardEyebrow}>DEMO DATA · NOT LIVE</Text><Text style={styles.cardTitle}>{vehicleLabel || 'Demo vehicle'}</Text><Text style={styles.muted}>{result.message}</Text><Text style={styles.muted}>This record is for testing the inspection flow only. It is not a decoded identity and should not be used as vehicle evidence.</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Use demo vehicle in inspection" style={styles.secondaryButton} onPress={() => onUse({ year: decodedVehicle?.year, make: decodedVehicle?.make, model: decodedVehicle?.model, vin: result.vin })}><Text style={styles.secondaryButtonText}>Use demo vehicle for testing</Text></TouchableOpacity></Card> : null}
